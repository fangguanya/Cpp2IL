...
public class MethodAnalysisContext : HasGenericParameters, IMethodInfoProvider, ISIL.IOperand
{
    ...
    // 仅显式诊断会话启用，正常生产路径不创建快照或持有取证对象。
    internal AnalysisStageRecorder? StageRecorder { get; set; }
    ...
    public void Analyze()
    {
        ...
        StageRecorder?.Capture("DominatorInfo", this);

        // Create locals
        SsaForm.Build(this);
        StageRecorder?.Capture("SsaForm.Build", this);
        LocalVariables.CreateAll(this);
        StageRecorder?.Capture("LocalVariables.CreateAll", this);
        ...
        StageRecorder?.Capture("CalleeSavedManagedReceiverRecovery.CaptureSsaCopyEvidence", this);
        ...
        StageRecorder?.Capture("RuntimeClassSlotIdentityRecovery.CaptureCandidates", this);
        ...
        StageRecorder?.Capture("FlagConditionRecovery.Run", this);
        DeadCodeEliminator.Run(this);
        StageRecorder?.Capture("DeadCodeEliminator.Run", this);
        ...
        StageRecorder?.Capture("MetadataResolver.ResolveAll", this);

        // ARM64大值类型通过X8指向调用方返回缓冲区；目标身份解析后才能把返回值绑定到真实栈局部。
        HiddenReturnBufferRecovery.Run(this);
        StageRecorder?.Capture("HiddenReturnBufferRecovery.Run", this);
        ...
        StageRecorder?.Capture("KeyFunctionRecovery.RewriteAllocationsAndBarriers", this);
        DeadCodeEliminator.Run(this);
        StageRecorder?.Capture("DeadCodeEliminator.Run", this);
        ...
        StageRecorder?.Capture("RuntimeMetadataSlotResolver.RecoverInitializedGenericCallTargets", this);
        InjectedCheckRemover.Run(this);
        StageRecorder?.Capture("InjectedCheckRemover.Run", this);
        // 中文注释：类初始化分支内部常带编译器注入的空引用检查；必须先删除注入异常边，
        // 再一次性裁除元数据与类初始化保护区，避免对同一 CFG 做重复保护区扫描。
        MetadataInitGuardRemover.Run(this);
        StageRecorder?.Capture("MetadataInitGuardRemover.Run", this);
        ...
        StageRecorder?.Capture("MetadataResolver.ResolvePost27TypeLoads", this);
        LocalVariables.ResolveTypesAndFields(this);
        StageRecorder?.Capture("LocalVariables.ResolveTypesAndFields", this);
        ...
        StageRecorder?.Capture("MetadataResolver.ResolveTypedPost27StringLoads", this);
        // 类型仍处于SSA局部时修复与托管签名冲突的实例接收者，避免后续简化把它折成地址操作数。
        ErasedInstanceReceiverRecovery.RunIncompatibleTypeReceivers(this);
        StageRecorder?.Capture("ErasedInstanceReceiverRecovery.RunIncompatibleTypeReceivers", this);
        // 值类型通过主不动点到达取址栈槽后，再把Object::Box唯一恢复为托管box。
        KeyFunctionRecovery.RewriteBoxing(this);
        StageRecorder?.Capture("KeyFunctionRecovery.RewriteBoxing", this);
        ...
        StageRecorder?.Capture("KeyFunctionRecovery.RewriteTypeTests", this);
        ...
        StageRecorder?.Capture("InlineTypeCheckRecovery.Run", this);
        AggregateStackCopyRecovery.RewriteResolvedCopies(this);
        StageRecorder?.Capture("AggregateStackCopyRecovery.RewriteResolvedCopies", this);
        ...
        StageRecorder?.Capture("ListEnumeratorCurrentRecovery.Run", this);

        // 接口与委托分派均依赖统一类型不动点；两者直接给重写后的返回局部变量写入精确类型。
        MetadataResolver.ResolveMethodRgctxCalls(this);
        StageRecorder?.Capture("MetadataResolver.ResolveMethodRgctxCalls", this);
        InterfaceDispatchRecovery.Run(this);
        StageRecorder?.Capture("InterfaceDispatchRecovery.Run", this);
        DelegateInvokeRecovery.Run(this);
        StageRecorder?.Capture("DelegateInvokeRecovery.Run", this);
        // 间接调用刚刚获得真实签名；立即把接收者、ref/out栈槽及其地址载体收敛为可生成CIL的精确类型。
        LocalVariables.ResolveLateCallTypesAndAddressCarriers(this);
        StageRecorder?.Capture("LocalVariables.ResolveLateCallTypesAndAddressCarriers", this);
        ...
        StageRecorder?.Capture("KeyFunctionRecovery.RewriteUnboxing", this);
        RuntimeMetadataSlotResolver.Run(this, initializedRuntimeMetadataSlots);
        StageRecorder?.Capture("RuntimeMetadataSlotResolver.Run", this);
        ...
        StageRecorder?.Capture("MetadataInitGuardRemover.RunRuntimeClassNullComparisons", this);
        // 中文注释：运行时类型槽到此才全部闭合；按最终 Newobj 类型一次性校准分配结果及其直接泛型调用。
        LocalVariables.RefreshResolvedNewobjTypesAndCalls(this);
        StageRecorder?.Capture("LocalVariables.RefreshResolvedNewobjTypesAndCalls", this);
        ...
        StageRecorder?.Capture("MetadataInitGuardRemover.RunMethodRgctxInitGuards", this);
        ...
        StageRecorder?.Capture("CallArgumentTrimmer.Run", this);
        BooleanFlagSimplifier.Run(this);
        StageRecorder?.Capture("BooleanFlagSimplifier.Run", this);

        // 中文注释：大值类型栈临时实参必须在 SSA 单一定义仍有效、槽写入尚未被折叠前整体
        // 证明并改写；一旦进入复制前递，基槽写入会被折进 AddressOf，其余槽写入将被死码删除。
        StackAggregateArgumentRecovery.Run(this);
        StageRecorder?.Capture("StackAggregateArgumentRecovery.Run", this);
        ...
        StageRecorder?.Capture("SsaSimplifier.Run", this);
        DeadCodeEliminator.Run(this);
        StageRecorder?.Capture("DeadCodeEliminator.Run", this);

        // 字符串常量槽与实例字段地址共享读取时，在入边身份尚存的最后阶段提交完整值 Phi。
        MetadataResolver.ResolveInitializedStringAndFieldPhis(this, initializedRuntimeMetadataSlots);
        StageRecorder?.Capture("MetadataResolver.ResolveInitializedStringAndFieldPhis", this);
        SsaForm.Remove(this);
        StageRecorder?.Capture("SsaForm.Remove", this);

        // Phi removal leaves a copy per merged version, most of which can share one local
        CopyCoalescer.Run(this);
        StageRecorder?.Capture("CopyCoalescer.Run", this);

        // 将无副作用的共享Move/Return尾链改写为直接返回，使大型字符串分派反编译为
        // 扁平早返回序列，避免数百层else触发Roslyn表达式复杂度上限。
        TailReturnRecovery.Run(this);
        StageRecorder?.Capture("TailReturnRecovery.Run", this);
        ...
        StageRecorder?.Capture("Simplifier.Simplify", this);
        ...
        var finalRuntimeClassSlotGroups = RuntimeClassSlotIdentityRecovery.BuildCurrentPlan(
            ...
            this,
            initializedRuntimeMetadataSlots);
        StageRecorder?.Capture("RuntimeClassSlotIdentityRecovery.BuildCurrentPlan", this);
        LocalVariables.ResolveRuntimeClassSlotFieldClosure(this, finalRuntimeClassSlotGroups);
        StageRecorder?.Capture("LocalVariables.ResolveRuntimeClassSlotFieldClosure", this);
        ...
        StageRecorder?.Capture("LocalVariables.ResolveExpectedSelfTypedStaticFields", this);
        // Fix float literals
        FloatLiteralRecovery.Run(this);
        StageRecorder?.Capture("FloatLiteralRecovery.Run", this);
        ...
        StageRecorder?.Capture("ManagedReferenceAddressRecovery.Run", this);
        LocalVariables.TypeAddressedLocals(this);
        StageRecorder?.Capture("LocalVariables.TypeAddressedLocals", this);

        // 浮点槽位类型到此已稳定；把只读 ELF 段的绝对标量加载恢复为源码字面量，
        // 可写静态存储仍保留为内存访问，不冻结其运行时状态。
        ReadOnlyScalarLiteralRecovery.Run(this);
        StageRecorder?.Capture("ReadOnlyScalarLiteralRecovery.Run", this);
        ...
        StageRecorder?.Capture("ErasedInstanceReceiverRecovery.RunScalarValueReceivers", this);
        ...
        StageRecorder?.Capture("CopyCoalescer.PruneUndefinedSourcePhiCopies", this);
        ...
        StageRecorder?.Capture("CopyCoalescer.ResolveNullReferencePhiCopyTypes", this);
        ...
        StageRecorder?.Capture("CalleeSavedManagedReceiverRecovery.ResolveManagedCopyCarrierTypesAndFields", this);
        InlineConstructorRecovery.Run(this);
        StageRecorder?.Capture("InlineConstructorRecovery.Run", this);
        ...
        StageRecorder?.Capture("StaticSingletonReceiverRecovery.Run", this);
        ...
        StageRecorder?.Capture("CalleeSavedManagedReceiverRecovery.Run", this);
        ...
        StageRecorder?.Capture("GenericCallRebinder.RunOwnedOpenSharedReceiverTargets", this);
        ...
        StageRecorder?.Capture("LocalVariables.ResolveLateVirtualScalarReturnSlotConflicts", this);
        ...
        StageRecorder?.Capture("LateVirtualCallRecovery.Run", this);
        ...
        StageRecorder?.Capture("CopyCoalescer.PruneIncompatiblePhiCopies", this);
        ...
        StageRecorder?.Capture("PropertyBackingFieldRecovery.Run", this);
        ...
        StageRecorder?.Capture("ManagedCallResultTypeRecovery.Run", this);
        // 中文注释：数值和取址槽位均已终态定型后执行布局恢复链；数组元素类型先闭合，
        // 随后的 List 快速路径可用同一具体 T 生成公开 Add/Clear/Count 调用。
        ArrayRecovery.Run(this);
        StageRecorder?.Capture("ArrayRecovery.Run", this);
        ListAddRecovery.Run(this);
        StageRecorder?.Capture("ListAddRecovery.Run", this);
        ListClearRecovery.Run(this);
        StageRecorder?.Capture("ListClearRecovery.Run", this);
        ListCountRecovery.Run(this);
        StageRecorder?.Capture("ListCountRecovery.Run", this);
        StringLengthRecovery.Run(this);
        StageRecorder?.Capture("StringLengthRecovery.Run", this);
        // 中文注释：长度专用操作数刚在上方生成；此处只把长度的 Int32 事实回填到循环归纳变量，
        // 不重复早期字段比较和原生位宽推导。
        LocalVariables.ResolveRecoveredLengthComparisonCarrierTypes(this);
        StageRecorder?.Capture("LocalVariables.ResolveRecoveredLengthComparisonCarrierTypes", this);
        EmptyArrayRecovery.Run(this);
        StageRecorder?.Capture("EmptyArrayRecovery.Run", this);
        ...
        StageRecorder?.Capture("LocalVariables.ResolveRecoveredPropertyScalarCarrierTypes", this);
        ...
        StageRecorder?.Capture("LocalVariables.ResolveFinalScalarCopyCarrierTypes", this);
        ...
        StageRecorder?.Capture("LocalVariables.ResolveFinalIntegerControlStateCarrierTypes", this);
        ...
        StageRecorder?.Capture("LocalVariables.ResolveFinalBooleanBitwiseCarrierTypes", this);
        ...
        StageRecorder?.Capture("LocalVariables.ResolveFinalBooleanBranchCarrierTypes", this);
        ...
        StageRecorder?.Capture("ConstantControlFlowRecovery.Run", this);
        ...
        StageRecorder?.Capture("ResolvedManagedCallScaffoldingRecovery.Run", this);
        ...
        StageRecorder?.Capture("PostSsaDeadStoreEliminator.Run", this);
        ...
        StageRecorder?.Capture("ListAddRecovery.RunCompactArrayAccess", this);
        DeadAddressCarrierRecovery.Run(this);
        StageRecorder?.Capture("DeadAddressCarrierRecovery.Run", this);

        // 所有退SSA常量、运行时类型、异常栈槽取址与保存寄存器接收者到此均已物化；
        // 这是三类直接辅助ABI唯一执行点，避免早期不完整操作数造成局部恢复和重复扫描。
        DirectRuntimeHelperRecovery.Run(this);
        StageRecorder?.Capture("DirectRuntimeHelperRecovery.Run", this);
        ...
        StageRecorder?.Capture("GenericCallRebinder.RunLateSharedReceiverTargets", this);
        ...
        StageRecorder?.Capture("GenericCallRebinder.RunLateSharedEnumClosure", this);

        // 中文注释：枚举泛型链闭合后，委托调用形参已获得最终 Func 类型；只在静态缓存
        // 空值门、唯一构造与唯一写回形成完整菱形时，把退 SSA 丢失的调用载体接回缓存字段。
        CachedDelegateCarrierRecovery.Run(this);
        StageRecorder?.Capture("CachedDelegateCarrierRecovery.Run", this);
        ...
        StageRecorder?.Capture("LocalVariables.ResolveFinalIntegerInductionCarrierTypes", this);

        LocalVariables.RemoveUnused(this);
        StageRecorder?.Capture("LocalVariables.RemoveUnused", this);
        ...
        StageRecorder?.Capture("MetadataResolver.ResolveFinalTypedAbsoluteStringLoads", this);
        MetadataResolver.ResolveInitializedInlineMetadataOperands(this, initializedRuntimeMetadataSlots);
        StageRecorder?.Capture("MetadataResolver.ResolveInitializedInlineMetadataOperands", this);
        StageRecorder?.Capture("AnalysisCompleted", this);
    }
